...
*/
[17:13, 30/10/2024] +56 9 9507 4938: / ejercicio 1
...
[17:13, 30/10/2024] +58 412-8828492: //1
const arraysNumeros = [2, 1, 4, 5, 6];
const suma1 = arraysNumeros.reduce((valorAnterior, valorActual) => {
  return valorAnterior + valorActual;
}, 0);
console.log(suma1);

//2
const arraysNumeros = [2, 1, 4, 5, 8];
const contador = (arraysNumeros.length);
const suma1 = arraysNumeros.reduce((valorAnterior, valorActual) => {
  return valorAnterior + valorActual;
}, 0);
console.log(suma1 / contador);

//3
var arrayNumeros = [342, 0, 500, 5, 233, 34, 76, 90, 23, 12, 11, 8, 3, 7]
function mayor(array){
  let comparador = array.sort(
    function (a,b) {return b-a;}
  ) 
  return comparador.slice(0,1)
}
console.log(mayor(arrayNumeros))
...
console.log(menor(arrayNumeros))

//5
function numerosPares(numeros, index = 0, pares = []) {
  if (index >= numeros.length) return pares;
  if (numeros[index] % 2 === 0) pares.push(numeros[index]);
  return numerosPares(numeros, index + 1, pares);
}
console.log(numerosPares([1, 2, 3, 4]));

//6
function numerosImpares(numeros, index = 0, impares = []) {
  if (index >= numeros.length) {return impares;
  } else if (numeros[index] % 2 !== 0) {
    impares.push(numeros[index]);
  }
  return numerosImpares(numeros, index + 1, impares);
}
console.log(numerosImpares([1, 2, 3, 4]));

//7
function ciudades(ciudades, index = 0, resultado = []) {
  if (index >= ciudades.length) return resultado;
  if (ciudades[index].startsWith("M")) resultado.push(ciudades[index]);
  return ciudadesConM(ciudades, index + 1, resultado);
}
console.log(ciudades(["Madrid", "Roma", "Milan"]));

//8
let nombres = ['peter', 'pedro', 'willian', 'osmel', 'jose', 'lexpin', 'juan']
function juanificador(nombres){
  if (nombres.includes('juan')){
    return nombres.indexOf('juan')
  }
}
console.log(juanificador(nombres))
...
// ` (buscar donde esta en el teclado)
